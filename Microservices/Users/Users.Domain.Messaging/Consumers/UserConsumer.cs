...
namespace Users.Domain.Messaging.Consumers
{
    public class UserConsumer : IConsumer<UserUpserted>
    {
        ...
        {
           await context.Publish(new Integration.Messaging.Outbound.Messages.UserUpserted{ Id = context.Message.Id, FullName = context.Message.FullName, Email = context.Message.Email });
        }
    }
}